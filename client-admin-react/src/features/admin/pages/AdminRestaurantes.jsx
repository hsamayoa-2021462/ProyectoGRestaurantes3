...
/* ─── SIDEBAR ─── */
function Sidebar({ collapsed, user, onLogout, onNavClick }) {
  return (
...
=======
          <div key={n.key} className={`ni${n.key === 'restaurantes' ? ' active' : ''}`}
            onClick={() => { if (onNavClick) onNavClick(); window.location.href = n.href; }}>
>>>>>>> a25b5b72d59fe4678a4f5d79da0e2c856b0c0184
...
/* Sidebar siempre fijo y abierto */
/* --- SIDEBAR RESPONSIVO --- */
.a-sidebar{
  width:var(--sidebar-w);
  background:var(--deep);
...
.sb-nav-label{font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--text-muted);padding:0 10px;margin:16px 0 8px;white-space:nowrap}
...
.a-main{flex:1;margin-left:var(--sidebar-w);min-height:100vh;display:flex;flex-direction:column}
...
@media(max-width:768px){
  .a-sidebar{transform:translateX(-100%);z-index:200;transition:transform 0.3s ease;}
...
.sidebar-container.open .a-sidebar{transform:translateX(0);}
...
/* Botón hamburguesa */
.hamburger-btn{display:none;align-items:center;justify-content:center;width:36px;height:36px;background:var(--glass-bg);border:1px solid var(--glass-bd);border-radius:10px;color:var(--text-muted);cursor:pointer;transition:all .2s;flex-shrink:0;}
.hamburger-btn:hover{color:var(--gold);border-color:rgba(201,168,76,.35);}

.sb-brand{padding:24px 20px 20px;border-bottom:1px solid var(--glass-bd);display:flex;align-items:center;gap:12px;flex-shrink:0;min-height:80px}
.sb-nav{flex:1;padding:16px 10px;overflow-y:auto}

/* --- MAIN CONTENT --- */
.a-main{flex:1;margin-left:var(--sidebar-w);min-height:100vh;display:flex;flex-direction:column;transition:margin-left 0.3s}
.a-top{height:64px;background:var(--deep);border-bottom:1px solid var(--glass-bd);display:flex;align-items:center;justify-content:space-between;padding:0 32px;position:sticky;top:0;z-index:50}

.a-cnt{padding:32px;flex:1;width:100%;max-width:1400px;margin:0 auto}

/* --- GRIDS FLEXIBLES --- */
.rg{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:20px}
.mesa-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px}

/* --- COMPONENTES --- */
.rc{background:var(--glass-bg);border:1px solid var(--glass-bd);border-radius:var(--radius-card);overflow:hidden;transition:transform .2s}
.sinp{width:100%;padding:10px 14px 10px 38px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:var(--radius-inp);color:var(--text)}

/* --- MEDIA QUERIES ADICIONALES --- */
@media(max-width:1024px){
  .a-cnt{padding:20px}
  .rg{grid-template-columns:repeat(auto-fill,minmax(240px,1fr))}
}
...
@media(max-width:480px){
  ...
  .sw{max-width:100%}
  .btn-p{justify-content:center}
  .mesa-grid{grid-template-columns:1fr}
}

/* Modales responsivos */
.mc{width:95% !important; margin: 10px auto}
.fg{grid-template-columns:1fr}

/* Helpers */
.sb-text, .sb-name {display:block}
...
=======
  ...
  {/* 2. MAIN: Asegúrate de que no haya otro Sidebar abajo */}
  <main className={`a-main${collapsed ? ' col' : ''}`}>
    <div className="a-top">
